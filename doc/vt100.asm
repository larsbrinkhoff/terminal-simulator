...
;
;; One iteration is 69 cycles.  The OUT instruction sends data to the
;; keyboard, which is transmitted in 3520 cycles.  Meanwhile the loop
;; spins, trying to transmit again.  When the keyboard UART has transmitted
;; the data, it will accept another round.  Since the scan bit is on,
;; the keyboard will respond back, so there will be an interrupt which
;; takes ~300 cycles to servide.
;;
;; The Technical manual says "Bell is generated by setting the bell
;; bit for 0.25 seconds (about 200 status words)."
X00d2:	lxi	b,X0fff         ;loop 4096 times
	ei                      ;enable interrupts
X00d6:	mvi	a,8
	ana	e
	mvi	a,7fh           ;LEDs, no beep
	jnz	X00e0           ;silent or not?
	mvi	a,0ffh          ;LEDs and beep
X00e0:	out	82h             ;send to keyboard
...
X00e8:	out	82h		; A = 0, so all LEDs off now
	lda	key_flags       ;flags should have a scan result
	ora	a
	jm	X00f5           ;keyboard error?
	mov	a,d             ;there was no interrupt from keyboard
	ori	4               ;error code = 4
	mov	d,a
X00f5:	push	d
...
X0269:	call	X03a2
	...
	ani	7fh
X0285:	sta	X21bd
	adi	30h             ;convert error code to ascii number
	lhld	cursor_address
	mov	m,a             ;display error code
	cpi	34h
	jz	X0298
	mvi	a,20h
	sta	X21a5           ;set local mode
X0298:	call	X0bf2
	lxi	b,X0011
	call	X0f7e
	jmp	X03ae
...
X03ae:	call	X1488
	...
	lda	X21a5
	ora	a
	jz	X03ae           ;jump if not local mode
	xra	a
	sta	key_locked
	jmp	X03ae
;;
...
X03cc:	push	psw
	...
	jz	X03ed
	mvi	c,1ah
	mvi	a,27h           ;tx+rx enable, dtr+rts on
	ori	10h             ;error reset
	out	1
X03ed:	mov	a,c
	cpi	7fh
...
X0538:	call	X11ce
	sta	X207a
X053e:	lxi	h,X2078         ;Check beep counter.
	mov	a,m
	ora	a
	jz	X054f           ;Jump if at zero.
	dcr	m               ;Else decrement.
	ani	4
	rrc
	rrc
	rrc
	sta	X2146           ;Store bell bit.
X054f:	lxi	h,X212c
	mov	a,m
	dcr	a
	jnz	X0569
...
X06c2:	mov	d,a             ;something in key_buffer?
	mvi	c,0
	mvi	b,4
	lxi	h,key_buffer+4
X06ca:	mov	a,m
...
X0747:	mov	a,c             ;count in key_flags
	cpi	4
	rp
	lxi	b,key_buffer
...
X076c:	mov	b,a             ;key debouncing
	lda	X2067           ;previous key
	cmp	b               ;same?
	mov	a,b
	sta	X2067           ;store in previous
	rnz
X0776:	pop	h
	sta	X2150           ;store debounced key
	cpi	7bh
	jz	X1a1a           ;SETUP
	cpi	6ah
	jz	X022c           ;NO-SCROLL
...
X0815:	lda	X2150
	...
	lxi	h,key_buffer+4
X081d:	cmp	m
	jz	X0841           ;key found in key_buffer+4
	inx	h
	dcr	d
	jnz	X081d
	mvi	d,4
...
X0838:	lda	X2150
	mov	m,a             ;store key in key_buffer+4
	mvi	a,0e1h
	sta	X2072
X0841:	xra	a
	lxi	h,key_flags
...
X0938:	lxi	h,X2078
	mov	a,m
	adi	8               ;Add 8 to beep counter.
	rc                      ;Unless it overflows.
	mov	m,a
	ret
...
X1f26:	push	b
	...
	jnz	X1f26
X1f36:	mvi	a,5
	out	1               ;tx+rx enable, dtr+rts off
	lda	X2158
	out	2
	mvi	a,25h
	sta	X21a5           ;set local mode + leds
	xra	a
	stc
	pop	d
	ret
...
X1f65:	mov	a,d
	call	X1f7b
	call	X1fa1           ;check modem control bits
	cmp	d
	jnz	X1f36
	dcr	d
	jnz	X1f65
...
X1fa1:	in	22h             ;modem control
	mov	b,a
	mvi	c,1             ;CD and CTS are set
	mov	a,b
	ani	90h             ;carrier detect and clear to send
	jz	X1fb3           ;jump if they are zero
	cpi	90h
	mvi	a,0ffh
	rnz                     ;return if they are not both set
	mvi	c,0             ;CD and CTS are zero
X1fb3:	mov	a,b
	ani	40h             ;speed indicator
	jz	X1fbd           ;jump if zero
	mov	a,c
	ori	2
	mov	c,a             ;speed indiator is set
X1fbd:	in	1
	rrc
	rrc
	cma
